...
(function($) {
    ...
    function calendar($el, options) {
        ...
        $el.on('click', '.js-cal-prev', function() {
            ...
            var $t = $(this),
                o = $t.data();
            if ($(this).hasClass('calendar-day current')) {
                var curr = new Date(o.date)
                var dd = ("0" + curr.getDate()).slice(-2)
                var mm = ("0" + (curr.getMonth() + 1)).slice(-2)
                var y = curr.getFullYear() % 2000;

                var formatted_date = dd + '-' + mm + '-' + y;
                console.log(formatted_date)
                window.location = '../logs/' + formatted_date + '.log'
            }
